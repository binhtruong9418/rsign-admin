...
export default function TemplateCreateOrEdit() {
    ...
    useEffect(() => {
        if (isEditMode && existingTemplate) {
            ...
            // Create placeholder File object from existing URL for edit mode
            const fileName = tmpl.fileUrl?.split('/').pop()?.split('?')[0] || tmpl.name || 'template.pdf';
            const placeholderFile = new File([], fileName, { type: 'application/pdf' });

            // 1. Recover Signers & Zones
            let restoredSigners: any[] = tmpl.signers || [];
            let restoredZones = (tmpl.signatureZones || []).map((z: any) => ({
                id: z.id || `zone-${Math.random().toString(36).substr(2, 9)}`,
                page: z.pageNumber || z.page || 1,
                x: z.x,
                y: z.y,
                width: z.width,
                height: z.height,
                label: z.label,
                signerId: 'signer-0' // Default, will update
            }));

            if (tmpl.signingMode === 'SHARED') {
                if (restoredSigners.length === 0) {
                    const labelMap = new Map<string, number>();
                    let signerCount = 0;

                    // Group zones by label to identify unique roles
                    ...
                    if (signerCount > 0) {
                        ...
                        restoredZones.forEach((z: any) => {
                            if (z.label && labelMap.has(z.label)) {
                                z.signerId = `signer-${labelMap.get(z.label)}`;
                            }
                        });
                    } else {
                        // Fallback: 1 Generic Signer if no labels
                        restoredSigners = [{ role: 'Signer 1', description: '', order: 1, color: '#3B82F6' }];
                        restoredZones.forEach((z: any) => z.signerId = 'signer-0');
                    }
                } else {
                    restoredZones.forEach((z: any) => {
                        const signerIndex = restoredSigners.findIndex((s: any) => s.role === z.label);
                        if (signerIndex >= 0) {
                            z.signerId = `signer-${signerIndex}`;
                        }
                    });
                }
            } else {
            ...
            // 2. Recover Signing Steps (for Sequential)
            let restoredSteps = tmpl.signingSteps || [];
            if (tmpl.signingFlow === 'SEQUENTIAL' && tmpl.signingSteps && tmpl.signingSteps.length > 0) {

                const firstStep = tmpl.signingSteps[0] as any;
                if (firstStep.zoneIndices && (!firstStep.signers || firstStep.signers.length === 0)) {
                    restoredSteps = tmpl.signingSteps.map((step: any) => {
                        const stepSigners: any[] = [];
                        const indices = step.zoneIndices || [];

                        // Find which signers are involved in this step based on zones
                        // This assumes we correctly mapped zones to signers above
                        const usedSignerIds = new Set<string>();
                        indices.forEach((zoneIdx: number) => {
                            if (restoredZones[zoneIdx]) {
                                usedSignerIds.add(restoredZones[zoneIdx].signerId);
                            }
                        });

                        // Map signerIds back to signer objects
                        usedSignerIds.forEach(sid => {
                            const idx = parseInt(sid.replace('signer-', ''));
                            if (restoredSigners[idx]) {
                                stepSigners.push(restoredSigners[idx]);
                            }
                        });

                        return {
                            stepNumber: step.stepOrder || step.stepNumber,
                            signers: stepSigners
                        };
                    });
                }
                setTemplateData({
                    name: tmpl.name || tmpl.title || tmpl.templateName || '',
